...
def main() -> int:
    # Catch Python activator mistakes before they can become another build loop.
    activator_path = HERE / "activate-ui-navigation-modernization2.py"
    try:
        ast.parse(activator_path.read_text(encoding="utf-8-sig"), filename=str(activator_path))
    except SyntaxError as exc:
        raise SystemExit(f"UI2 verification: activator syntax error line {exc.lineno}: {exc.msg}")
    ...
    # Keep the helper header-only: no project/linker entry should be necessary.
