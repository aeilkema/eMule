...
def main() -> int:
    ...
    require(scheduler, "event.fileHash = key;", "telemetry stable file identity")
    ...
    require(scheduler, "m_telemetry.MarkAppliedIntervention(fileHash, fileName);", "delayed telemetry identity handoff")
    ...
    require(history, "worker-local buffer first", "history startup lock isolation")
    ...
    require(telemetry, "file_hash BLOB", "scheduler decision file hash schema")
    require(telemetry, "ALTER TABLE scheduler_decisions ADD COLUMN file_hash BLOB", "preview database file-hash migration")
    require(telemetry, "idx_scheduler_decisions_hash_applied", "file-hash applied lookup index")
    require(telemetry, "m_persistAppliedQueue", "delayed applied-state queue")
    require(telemetry, "QueueAppliedPersist(fileHash, fileName)", "delayed applied persistence enqueue")
    require(telemetry, "UPDATE scheduler_decisions SET applied=1", "durable applied-state update")
    require(telemetry, "file_hash=?1", "hash-keyed durable applied-state match")
    require(telemetry, "file_hash IS NULL AND file_name=?2", "legacy name fallback for old telemetry rows")
