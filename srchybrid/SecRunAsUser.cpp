...
eResult CSecRunAsUser::PrepareUser()
{
	...
	try {
		...
		try {
			...
			ADSPath.Format(L"WinNT://%s,computer", (LPCWSTR)cscompName);
...
bool CSecRunAsUser::CreateEmuleUser(IADsContainerPtr pUsers)
{
	...
	pUser->put_Description(CStringW(L"Account used to run eMule with additional security").AllocSysString());
	pUser->SetInfo();
	m_strPassword = CreateRandomPW();
...
bool CSecRunAsUser::SetObjectPermission(const CString &strDirFile, DWORD lGrantedAccess)
{
	...
	try {
		...
		DWORD cbDomain = 0;
		DWORD cbUserSID = 0;
		fAPISuccess = ::LookupAccountNameW(NULL, EMULEACCOUNTW, pUserSID, &cbUserSID, szDomain, &cbDomain, &snuType);
		...
		szDomain = (WCHAR*)MHeapAlloc(cbDomain * sizeof(WCHAR));
		if (!szDomain)
			throwCStr(_T("Run as unprivileged user: Error: Allocating memory failed,"));
...
eResult CSecRunAsUser::RestartAsUser()
{
	...
	WCHAR szAppPath[MAX_PATH + 2];
	DWORD dwModPathLen = ::GetModuleFileNameW(NULL, &szAppPath[1], MAX_PATH);
	if (dwModPathLen == 0 || dwModPathLen == _countof(szAppPath))
		return RES_FAILED;
	szAppPath[0] = L'"';
	szAppPath[++dwModPathLen] = L'"';
	szAppPath[++dwModPathLen] = L'\0';
...
{
	return m_strCurrentUser.IsEmpty() ? CStringW(L"Unknown") : m_strCurrentUser;
}
