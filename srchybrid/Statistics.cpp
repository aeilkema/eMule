...
float CStatistics::GetAvgDownloadRate(int averageType)
{
	...
	case AVG_TOTAL:
		if (theStats.transferStarttime > 0) {
			...
			if (running >= 5)
				return ((float)theStats.sessionReceivedBytes / (1.024f * (float)running) + thePrefs.GetConnAvgDownRate()) / 2.0f;
		}
	...
	case AVG_SESSION:
		if (theStats.transferStarttime > 0) {
			DWORD running = ::GetTickCount() - theStats.transferStarttime;
			if (running >= 5)
				return (float)theStats.sessionReceivedBytes / (1.024f * (float)running);
		}
		break;
	case AVG_TIME:
		if (!downrate_hist.IsEmpty()) {
			...
			if (running > 0)
				return (float)(downrate_hist.Tail().datalen - downrate_hist.Head().datalen) / (1.024f * (float)running);
		}
	}
	return 0.0f;
}
...
float CStatistics::GetAvgUploadRate(int averageType)
{
	...
	case AVG_TOTAL:
		if (theStats.transferStarttime > 0) {
			DWORD running = ::GetTickCount() - theStats.transferStarttime;
			if (running >= 5)
				return ((float)theStats.sessionSentBytes / (1.024f * (float)running) + thePrefs.GetConnAvgUpRate()) / 2.0f;
		}
	...
	case AVG_SESSION:
		if (theStats.transferStarttime) {
			DWORD running = ::GetTickCount() - theStats.transferStarttime;
			if (running >= 5)
				return (float)theStats.sessionSentBytes / (1.024f * (float)running);
		}
		break;
	case AVG_TIME:
		if (!uprate_hist.IsEmpty()) {
			...
			if (running > 0)
				return (float)(uprate_hist.Tail().datalen - uprate_hist.Head().datalen) / (1.024f * (float)running);
		}
	}
	return 0.0f;
}
