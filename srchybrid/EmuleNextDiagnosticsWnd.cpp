...
namespace
{
    enum { IDC_EN_DIAG_REFRESH = 0x7ED0, IDC_EN_DIAG_CHECK, IDC_EN_DIAG_BACKUP, IDC_EN_DIAG_RESTORE,
        IDC_EN_DIAG_PRUNE, IDC_EN_DIAG_CHECKPOINT, IDC_EN_DIAG_OPEN };
...
int CEmuleNextDiagnosticsWnd::OnCreate(LPCREATESTRUCT createStruct)
{
    ...
    if (!m_title.Create(_T("Database & Recovery"), WS_CHILD | WS_VISIBLE | SS_LEFT, empty, this)
        || !m_subtitle.Create(_T("Integrity, backups, recovery, maintenance and writer-queue diagnostics."), WS_CHILD | WS_VISIBLE | SS_LEFT, empty, this)
        ...
        || !m_actionStatus.Create(_T("Maintenance actions run outside the GUI thread."), WS_CHILD | WS_VISIBLE | SS_LEFT, empty, this)) return -1;
    CFont* font = CFont::FromHandle(static_cast<HFONT>(::GetStockObject(DEFAULT_GUI_FONT)));
    CWnd* controls[] = { &m_title, &m_subtitle, &m_health, &m_details, &m_refresh, &m_check, &m_backup, &m_restore, &m_prune, &m_checkpoint, &m_openBackups, &m_actionStatus };
...
{
    const BOOL e = m_busy ? FALSE : TRUE; m_refresh.EnableWindow(e); m_check.EnableWindow(e); m_restore.EnableWindow(e); m_openBackups.EnableWindow(e);
...
void CEmuleNextDiagnosticsWnd::LayoutControls(int cx, int cy)
{
    ...
    const int perRow = cx < CEmuleNextUiMetrics::Scale(m_hWnd, 900) ? 3 : 7; CButton* buttons[] = { &m_refresh, &m_check, &m_backup, &m_restore, &m_prune, &m_checkpoint, &m_openBackups };
    const int buttonWidth = max(CEmuleNextUiMetrics::Scale(m_hWnd, 110), (clientWidth - gap * (perRow - 1)) / perRow), rows = (_countof(buttons) + perRow - 1) / perRow;
