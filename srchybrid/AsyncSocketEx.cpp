		...
		if (message >= WM_SOCKETEX_NOTIFY) {
			...
			if (message < static_cast<UINT>(WM_SOCKETEX_NOTIFY + pWnd->m_nWindowDataSize)) { //Index is within socket storage
				...
				if (pSocket->m_pFirstLayer) {
					...
					//Dispatch notification to CAsyncSocketEx instance
					switch (nEvent) {
					case FD_READ:
					...
					case FD_CONNECT:
						...
						if (!nErrorCode && pSocket == pWnd->m_pAsyncSocketExWindowData[message - WM_SOCKETEX_NOTIFY].m_pSocket) {
